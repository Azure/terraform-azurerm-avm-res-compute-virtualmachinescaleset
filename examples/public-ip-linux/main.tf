...
module "terraform_azurerm_avm_res_compute_virtualmachinescaleset" {
  ...
  instances = 2
  # A Microsoft.Network API version newer than the module default is required by
  # some Public IP features, such as the `StandardV2` SKU.
  ...
  network_interface = [{
    ...
    ip_configuration = [{
      name      = "VMSS-IPConfig"
      subnet_id = azurerm_subnet.subnet.id
      # Assigns a Public IP Address to each Virtual Machine Scale Set instance.
      # The SKU name and tier map to the two separate Azure Resource Manager
      # fields, so `Standard_Regional` is not a valid `sku_name` here.
      public_ip_address = [{
        name                    = "VMSS-PIP"
